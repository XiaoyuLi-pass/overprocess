...
class IrMutator(
    ...
    fun mutate(program: IrProgram): Boolean {
        println("=== MUTATE METHOD CALLED ===")  // 添加这行

        val configByMethods = this::class.declaredMemberFunctions.filter {
            it.annotations.any { anno -> anno.annotationClass == ConfigBy::class }
        }

        println("Available mutators: ${configByMethods.map { it.name }}")  // 添加这行
        ...
        val weights = configByMethods.map { method ->
            ...
            println("  ${method.name}: weight=$weight")  // 添加这行
            weight
        }

        if (weights.all { it == 0 }) {
            println("WARNING: All weights are zero!")  // 添加这行
            return false
        }

        val mutatorMethod = rouletteSelection(configByMethods, weights, random)
        println("Selected: ${mutatorMethod.name}")  // 添加这行
